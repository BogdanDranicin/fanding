...
type Session struct {
	...
	// Misses — сколько тактов подряд робот промолчал к этому моменту. У живой
	// сессии всегда ноль: первый же пропуск снимает робота со страницы. Ненулевым
	// поле остаётся у строки истории — по нему видно, что серию закрыл пропуск,
	// а не молчание всей ленты.
...
// со страницы.
//
// Единица, а не двойка (09.09.2026). Прежний допуск в один пропуск задумывался как
// снисхождение к роботу, которому не нашлось ликвидности, но на странице он читался
// иначе: строка желтела и продолжала висеть среди работающих, а рядом с ней шёл
// обратный отсчёт до удара, которого не будет. Список «Сейчас» отвечает на вопрос
// «кто печатает прямо сейчас», и робот, не отработавший ожидаемый такт, на него уже
// не ответ. Пропустивший уходит в историю тем же обходом, в котором пропуск замечен;
// вернувшись, он заводится заново — чёрный список dropped держится ровно до тех пор,
// пока серия не уедет из окна анализа.
const MaxMisses = 1
...
//
// Окно сравнения — час, но не больше времени жизни самой серии. Иначе обе части
// отношения меряют разное время: у робота, работающего пять минут, минутный поток
// растягивался на час, которого не было, и страница показывала силу в тысячу
// процентов (замер на вечерней сессии 28.08: SIH7, один лот раз в 12 с на пустом
// контракте — 1236 %). При равных окнах доля больше сотни быть не может.
// baseMinutes — сколько последних минут берётся в знаменатель силы: время жизни
// серии, но не больше часа и не меньше минуты.
func (s Session) baseMinutes() int {
	minutes := int(s.DurationSec()/60) + 1
	if minutes < 1 {
		minutes = 1
	}
...
//
// Предварительные находки в базу не едут. На валюте робот заявляется со второго
// принта, а второй принт — это ещё не робот, а гипотеза: подтвердится она или
// рассыплется, станет ясно через такт. Пока такие находки писались наравне со
// всеми, история за сутки состояла из них на девять десятых (замер на проде
// 28.08: 118 строк «EURRUBF, 4 лота, 243 с» — одна и та же случайная пара,
// найденная заново каждые несколько секунд), и настоящих роботов в ней было не
// разглядеть. На живой странице они по-прежнему видны сразу — она берёт срез из
// памяти, а не из базы, и скорость показа от этого не страдает.
//
// Пометка с неподтверждённых не снимается: дорастёт серия до подтверждённой —
// уедет в базу целиком, вместе со своим началом.
func (r *registry) takeDirty() []*Session {
	var out []*Session
	for _, s := range r.sessions {
		if s.dirty && !s.Provisional {
			s.dirty = false
			out = append(out, s)
		}
	}
	// Закрытые разбираем до конца в любом случае: неподтверждённая серия, которая
	// уже кончилась, подтвердиться не может, и держать её в очереди незачем.
