...
type Config struct {
	...
	// интервалов, легших в период. Это пороги для всего, кроме валюты.
	//
	// Держатся на длине, с которой периодичность уже не объясняется совпадением:
	// на коротких сериях детектор собирает «роботов» из случайных сделок, и на
	// ленте всего рынка это давало около сотни ложных находок одновременно
	// (замер: 96 находок на случайной ленте из 100 тысяч принтов по 150 тикерам).
	MinPrints int
	MinBeats  int

	// MinPrintsCurrency/MinBeatsCurrency — пороги для валютных инструментов: там
	// ликвидность собрана в нескольких контрактах, и ждать шестого принта значит
	// увидеть робота на несколько тактов позже, чем он начал работать.
	//
	// Третий принт, а не второй. Замер на случайных лентах (20 прогонов по 900
	// принтов) показывает цену каждого шага: порог 2 даёт 1047 ложных находок,
	// порог 3 — две, порог 4 — ни одной. Платим за это одним тактом ожидания:
	// робот с периодом 30 с виден через минуту вместо получаса секунд. На двух
	// принтах серия — это один интервал, и проверять период нечем, кроме попадания
	// в границы; на трёх появляется второй интервал и разброс между ними.
	//
	// Находки короче ConfidentPrints всё равно помечаются Provisional, а отсев
	// оставшихся ложных ложится на правило пропущенных тактов в реестре сессий.
...
func DefaultConfig() Config {
	return Config{
		...
		MinPrints:         ConfidentPrints,
		MinBeats:          ConfidentPrints - 1,
		MinPrintsCurrency: 3,
		MinBeatsCurrency:  2,
		QtyTolRel:         0,
		QtyTolAbs:         1,
