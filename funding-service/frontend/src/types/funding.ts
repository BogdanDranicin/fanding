export interface InstrumentFunding {
   ...
   * (она отстаёт на четверть часа), `iss-rebuild` — той же лентой, перечитанной
   * за день целиком, когда оба потока окно не закрыли, `voltoday` —
   * приближением по приросту дневного объёма.
   */
  settl_source?: string;
